...
3. [x] 实现 Provider profile CRUD，数据库只保存 credential reference；Product validate `32119057930` 已通过。
4. [x] 实现 Windows Credential Manager SecretStore 与 DPAPI current-user 后备；Product validate `32127969715` 的 lifecycle、CredMan、DPAPI round-trip/tamper、Clippy 与 release build 全部通过。
