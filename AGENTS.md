...
- 完成可交付的源码或产品功能后，执行者必须自动提交、推送，并在必要验证通过后通过统一 Release workflow 自动递增预览版本、创建 GitHub Release 和上传可下载资产；不得停在“已验证但未发版”。文档-only、测试-only、CI-only 或用户明确标注不发版的改动可只提交推送。
- 自动发版授权包含创建预览版 Release 和发布安装器、便携 ZIP、独立 EXE 与校验文件；不包含长期公开、正式稳定版语义、运行与当前任务无关的 workflow，或在审计发现 secret、新敏感历史、未知外部输入、许可证冲突、数据破坏风险等新增实质风险后继续。这些情况必须先请求用户确认。
...
- 内置搜索端点当前可能返回 HTTP 404。遇到不可用时不要反复空耗重试；应主动发现并优先使用可用的 MCP 检索工具，必要时改用官方 API 或官方仓库 CLI，并保持来源为官方或一手资料。
